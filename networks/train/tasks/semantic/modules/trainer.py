...
from tasks.semantic.modules.FourDenoiseNet import *
#from tasks.semantic.modules.WeatherNet import *
...
class Trainer():
    def __init__(self, ARCH, DATA, datadir, logdir, path=None):
        ...
        if self.path is not None:
            ...
            w_dict = torch.load(path + "/FourDenoiseNet",
    ...
    def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10,
        ...
        for i, (in_vol, pre_in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _) in enumerate(train_loader):
            ...
            output = model(in_vol, pre_in_vol)
    ...
    def validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans):
        ...
        with torch.no_grad():
            ...
            for i, (in_vol, pre_in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _) in enumerate(val_loader):
                if not self.multi_gpu and self.gpu:
                    in_vol = in_vol.cuda()
